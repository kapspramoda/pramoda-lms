...
export default function VideoLessonsPage() {
  ...
  const bgMain = isDarkMode ? "bg-slate-950 text-slate-100" : "bg-blue-50/30 text-gray-800";
  const bgCard = isDarkMode ? "bg-slate-900 border-slate-800" : "bg-white border-blue-100 shadow-sm";
  ...
  const inputBg = isDarkMode ? "bg-slate-800 border-slate-700 text-white" : "bg-blue-50 border-blue-100 text-blue-900";
  ...
  return (
    <div className={`font-sans flex h-screen overflow-hidden transition-colors duration-300 ${bgMain}`}>
      ...
      <aside className={`w-64 bg-blue-900 text-white flex flex-col fixed inset-y-0 left-0 z-[9999] transform ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 md:static transition-transform duration-300 shadow-2xl`}>
        <div onClick={() => router.push('/')} className="p-6 border-b border-blue-800 font-bold text-xl tracking-wider cursor-pointer hover:opacity-80 transition flex items-center gap-2">
          
          PramodaChemistry<span className="text-blue-300"></span>
        </div>      
        ...
        <nav className="flex-1 p-4 space-y-2 overflow-y-auto custom-scrollbar">
          ...
          <a href="#" onClick={(e) => { e.preventDefault(); router.push('/videos'); }} className="flex items-center space-x-3 bg-blue-800 px-4 py-3 rounded-lg transition shadow-inner border border-blue-800/30">
        ...
        <div className="p-4 border-t border-blue-800">
          <button onClick={handleLogout} className="w-full flex items-center space-x-3 hover:bg-red-500 text-blue-200 hover:text-white p-3 rounded-xl transition">
            <div className="w-8 h-8 rounded-full bg-blue-950 flex items-center justify-center text-white font-bold text-sm">
              {userName.charAt(0)}
            </div>
          ...
          <div className="flex items-center space-x-4 md:space-x-6">
            ...
            <div className={`flex items-center gap-3 px-4 py-2 rounded-full border ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-blue-50 border-blue-100'}`}>
              <div className="text-right hidden sm:block">
                <p className={`font-bold text-sm ${isDarkMode ? 'text-blue-400' : 'text-blue-900'}`}>{userYear} A/L</p>
              </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            {loading ? (
              ...
              <div className="animate-fade-in space-y-4">
                ...
                >
                  ...
                  <div className="relative w-full h-full flex-grow">
                    ...
                    <div className="absolute bottom-0 left-0 w-full h-[60px] md:h-[65px] z-[99] bg-black pointer-events-auto flex items-center justify-end px-4 md:px-5">
                      <span className="text-[10px] md:text-xs font-bold text-slate-500/80 mt-2 mr-1">Pramoda Chemistry</span>
                    </div>
                ...
                <div className={`p-4 md:p-5 rounded-2xl border shadow-sm flex flex-col gap-4 transition-all ${isDarkMode ? 'bg-slate-800/50 border-blue-900/30' : 'bg-blue-50/40 border-blue-100/50'}`}>
                  ...
                  <div className="flex flex-col xl:flex-row xl:items-center xl:justify-between gap-4">
                    ...
                    <div className="flex items-center gap-2 md:gap-3 flex-wrap">
                      <div className={`flex items-center rounded-xl border overflow-hidden shadow-sm flex-shrink-0 ${isDarkMode ? 'bg-slate-800 border-slate-600' : 'bg-white border-blue-200'}`}>
                        <button onClick={togglePlay} className={`px-4 py-2.5 md:py-3 transition-colors ${isDarkMode ? 'hover:bg-slate-700 text-blue-400' : 'hover:bg-blue-50 text-blue-700'}`} title={isPlaying ? "නවත්වන්න" : "ධාවනය කරන්න"}>
                          {isPlaying ? (
                        ...
                        <button onClick={skipBackward} className={`px-3 py-2.5 md:py-3 transition-colors border-l ${isDarkMode ? 'hover:bg-slate-700 border-slate-600 text-slate-300' : 'hover:bg-blue-50 border-blue-200 text-slate-700'}`} title="තත්පර 10ක් ආපස්සට">
                        ...
                        <button onClick={skipForward} className={`px-3 py-2.5 md:py-3 transition-colors border-l ${isDarkMode ? 'hover:bg-slate-700 border-slate-600 text-slate-300' : 'hover:bg-blue-50 border-blue-200 text-slate-700'}`} title="තත්පර 10ක් ඉදිරියට">
                      ...
                      <button onClick={toggleSpeed} className={`px-4 py-2.5 md:py-3 rounded-xl border font-bold text-sm transition-colors shadow-sm flex-shrink-0 ${isDarkMode ? 'bg-slate-800 border-slate-600 text-amber-400 hover:bg-slate-700' : 'bg-white border-blue-200 text-amber-600 hover:bg-blue-50'}`} title="ධාවන වේගය වෙනස් කරන්න">
                    ...
                    <div className="flex items-center gap-2 md:gap-3 flex-wrap">
                      <div className={`flex items-center rounded-xl border overflow-hidden shadow-sm flex-shrink-0 ${isDarkMode ? 'bg-slate-800 border-slate-600' : 'bg-white border-blue-200'}`}>
                        <button onClick={handleVolumeDown} className={`px-3 py-2.5 md:py-3 transition-colors ${isDarkMode ? 'hover:bg-slate-700 text-slate-300' : 'hover:bg-blue-50 text-slate-700'}`}>
                        ...
                        <button onClick={handleToggleMute} className={`px-3 py-2.5 md:py-3 transition-colors border-x ${isDarkMode ? 'hover:bg-slate-700 border-slate-600' : 'hover:bg-blue-50 border-blue-200'}`}>
                          {isMuted ? (
                        ...
                        <button onClick={handleVolumeUp} className={`px-3 py-2.5 md:py-3 transition-colors ${isDarkMode ? 'hover:bg-slate-700 text-slate-300' : 'hover:bg-blue-50 text-slate-700'}`}>
                      ...
                      {currentVideo.tuteUrl ? (
                        <a href={getDownloadUrl(currentVideo.tuteUrl)} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-blue-600 to-cyan-600 hover:from-blue-700 hover:to-cyan-700 text-white px-4 py-2.5 md:px-5 md:py-3 text-xs md:text-sm font-bold transition-all shadow-sm flex-shrink-0">
                ...
                <div className={`p-5 rounded-2xl border ${bgCard}`}>
                  <div className="flex flex-wrap items-center gap-3 mb-3">
                    <span className="inline-block text-xs px-3 py-1 rounded-full font-bold bg-blue-600 text-white uppercase tracking-wider">
          ...
          {videos.length > 0 && (
            <div className="space-y-6">
              ...
              <div className={`${bgCard} border p-4 rounded-2xl`}>
                <label className="block text-xs font-black uppercase tracking-wider mb-2 text-blue-500">🔖 විෂය නිර්දේශයේ පාඩම් අනුව තෝරන්න</label>
                <select 
                  value={selectedLesson} 
                  onChange={(e) => setSelectedLesson(e.target.value)}
                  className={`w-full rounded-xl p-3 text-sm font-bold outline-none focus:ring-2 focus:ring-blue-500/30 transition border ${inputBg}`}
                >
              ...
              <div className={`${bgCard} border p-5 rounded-3xl h-fit max-h-[70vh] flex flex-col`}>
                <h3 className="text-base font-black mb-4 border-b pb-4 flex items-center justify-between">
                  ...
                  <span className="bg-blue-100 text-blue-800 text-xs font-extrabold px-2.5 py-1 rounded-md">{filteredVideos.length}</span>
                </h3>
                    ...
                    <div 
                      ...
                      className={`flex gap-3 p-2.5 rounded-2xl cursor-pointer transition border ${
                        currentVideo?._id === video._id 
                          ? 'bg-blue-600/10 border-blue-400 shadow-sm text-blue-500 font-bold' 
                          : 'border-transparent hover:bg-blue-50/20 hover:border-blue-100/30'
                      }`}
                    >
                      <div className="w-28 h-16 bg-slate-800 rounded-xl overflow-hidden flex-shrink-0 relative shadow-sm">
                        ...
                        />
                        {currentVideo?._id === video._id && (
                          <div className="absolute inset-0 bg-blue-600/70 flex items-center justify-center backdrop-blur-[1px]">
                      ...
                      <div className="flex-1 py-0.5 flex flex-col justify-center">
                        <h4 className={`text-xs line-clamp-2 leading-tight ${currentVideo?._id === video._id ? 'text-blue-600 font-black' : ''}`}>
                          {video.title}
                        </h4>
