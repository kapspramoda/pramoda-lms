            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 animate-fade-in">
              {markings.map(mark => (
                <div key={mark._id} className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 flex flex-col sm:flex-row justify-between items-start sm:items-center hover:shadow-md transition gap-4">
                  <div>
                    <span className="text-xs font-bold uppercase px-3 py-1 rounded-full mb-2 inline-block bg-blue-100 text-blue-700">
                      {mark.category}
                    </span>
                    <h3 className="text-lg font-bold text-gray-800 leading-tight">{mark.title}</h3>
                  </div>
                  <a href={mark.pdfUrl} target="_blank" rel="noopener noreferrer" className="bg-blue-600 text-white px-6 py-3 rounded-xl font-bold hover:bg-blue-700 transition shadow-sm border border-blue-700 flex items-center gap-2 w-full sm:w-auto justify-center">
                    <span>📥</span> Download
                  </a>
                </div>
              ))}
