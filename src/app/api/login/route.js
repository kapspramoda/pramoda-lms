...
// User Model (email වෙනුවට username යොදා ඇත)
const UserSchema = new mongoose.Schema({
  name: String,
  username: { type: String, unique: true }, // WhatsApp Number
  password: String,
  alYear: String
});
...
export async function POST(req) {
  try {
    ...
    // ඊමේල් එක වෙනුවට WhatsApp අංකයෙන් (username) දත්ත සමුදායේ සෙවීම
    ...
    if (user.password.startsWith('$2a$') || user.password.startsWith('$2b$')) {
      ...
      // Password එක Hash කර නොමැති නම්:
      isPasswordMatch = (user.password === password);
    }
    ...
    return NextResponse.json(
        { message: 'සාර්ථකයි', user: { name: user.name, username: user.username, alYear: user.alYear } }, 
        { status: 200 }
      );
